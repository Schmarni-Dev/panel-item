...
fn main() {
    ...
    if let Err(e) = generate_protocol_file(protocols, &output_dir.join("acceptor.rs"), Side::Client)
    {
        eprintln!("Failed to generate protocol files: {e}");
    }
    let protocols = get_all_protocols();
    if let Err(e) = generate_protocol_file(protocols, &output_dir.join("provider.rs"), Side::Server)
    {
        eprintln!("Failed to generate protocol files: {e}");
    }
...
pub fn get_all_protocols() -> Vec<ProtocolInfo> {
    vec![
        ...
        ProtocolInfo {
            name: "panel_item",
            content: include_str!("./schemas/panel_item.kdl"),
            generate_node: true,
            partial_eq: true,
        },
    ]
...
pub fn generate_protocol_file(
    ...
    let protocols = protocols
        .into_iter()
        .map(|p| {
            let protocol = p.0.tokenize(p.1.generate_node, p.1.partial_eq, side);
...
trait Tokenize {
    fn tokenize(&self, generate_node: bool, partial_eq: bool, side: Side) -> TokenStream;
}
impl Tokenize for Protocol {
    fn tokenize(&self, generate_node: bool, partial_eq: bool, side: Side) -> TokenStream {
        ...
        let custom_enums = self
            .custom_enums
            .iter()
            .map(|e| e.tokenize(generate_node, partial_eq, side));
        let custom_unions = self
            .custom_unions
            .iter()
            .map(|u| u.tokenize(generate_node, partial_eq, side));
        let custom_structs = self
            .custom_structs
            .iter()
            .map(|s| s.tokenize(generate_node, partial_eq, side));
        let aspects = self
            .aspects
            .iter()
            .map(|a| a.tokenize(generate_node, partial_eq, side));
        let interface = self
            .interface
            .as_ref()
            .map(|p| {
                p.members
                    .iter()
                    .map(|m| generate_server_member(Some(p.node_id), 0, "Interface", m, side))
...
impl Tokenize for CustomEnum {
    fn tokenize(&self, _generate_node: bool, partial_eq: bool, _side: Side) -> TokenStream {
...
impl Tokenize for CustomUnion {
    fn tokenize(&self, _generate_node: bool, partial_eq: bool, side: Side) -> TokenStream {
        let name = Ident::new(&self.name.to_case(Case::Pascal), Span::call_site());
        let description = &self.description;
        ...
        let option_decls = self
            .options
            .iter()
            .map(|e| e.tokenize(_generate_node, partial_eq, side));
...
impl Tokenize for UnionOption {
    fn tokenize(&self, _generate_node: bool, _partial_eq: bool, _side: Side) -> TokenStream {
        let name = self
            .name
            .as_ref()
...
impl Tokenize for Aspect {
    fn tokenize(&self, generate_node: bool, _partial_eq: bool, side: Side) -> TokenStream {
        ...
        let aspect_events = {
            let variants = self.members.iter().filter(|m| m.side == side).map(|m| {
        ...
        let aspect_event_sender_impl = if self.members.iter().any(|m| m.side == side) {
            generate_event_sender_impl(self, side)
        } else {
            quote!()
        };
        ...
        let server_side_members =
            server_members.map(|m| generate_server_member(None, self.id, &self.name, m, side));

        fn aspect_filter(a: &Aspect, side: Side) -> bool {
            a.name.to_case(Case::Snake) != "owned" && a.members.iter().any(|m| m.side == side)
        }
        ...
        let impl_traits = self
            ...
            .map(|a| {
                ...
                if a.members.iter().any(|m| m.side == side) {
        ...
        let node = generate_node.then(|| quote! {
			...
			impl #node_name {
				pub(crate) fn from_id(client: &std::sync::Arc<crate::panel_item::PanelItemHandle>, id: u64, owned: bool) -> Self {
...
fn generate_event_sender_impl(aspect: &Aspect, side: Side) -> TokenStream {
    ...
    let (signal_matches, method_matches): (Vec<_>, Vec<_>) = aspect
		...
		.map(|member| -> (MemberType, TokenStream) {
			...
			let deserialize_types = member
				...
				.iter()
				.map(|a| {
                                    let t = generate_argument_type(&convert_deserializeable_argument_type(&a._type), true);
                                    if a.optional {
                                        quote! {Option<#t>}
                                    } else {
                                        t
    ...
    quote! {
        impl crate::scenegraph::EventParser for #event_name {
            ...
            fn parse_signal(_client: &std::sync::Arc<crate::panel_item::PanelItemHandle>, signal_id: u64, _data: &[u8], _fds: Vec<std::os::fd::OwnedFd>) -> Result<Self, stardust_xr_wire::scenegraph::ScenegraphError> {
                match signal_id {
                    #(#signal_matches)*
                    _ => Err(stardust_xr_wire::scenegraph::ScenegraphError::MemberNotFound),
                }
            }
            fn parse_method(_client: &std::sync::Arc<crate::panel_item::PanelItemHandle>, method_id: u64, _data: &[u8], _fds: Vec<std::os::fd::OwnedFd>, response: stardust_xr_wire::messenger::MethodResponse) -> Result<Self, stardust_xr_wire::scenegraph::ScenegraphError> {
                match method_id {
                    #(#method_matches)*
                    _ => {
...
fn generate_server_member(
    ...
    let first_arg = if interface_node_id.is_some() {
        quote!(_client: &std::sync::Arc<crate::panel_item::PanelItemHandle>)
    } else {
        quote!(&self)
    };
    ...
    let argument_decls = member
        .arguments
        .iter()
        .map(|a| generate_argument_decl(a, member.side != side))
    ...
    match _type {
        MemberType::Signal => {
            let mut body = if let Some(interface_node_id) = &interface_node_id {
                quote! {
                    let data: (#argument_types) = (#(#argument_uses),*);
            ...
            if interface_node_id.is_some() {
                return quote! {
                    #[doc = #description]
                    pub fn #name(#argument_decls) -> stardust_xr_fusion::node::NodeResult<#return_type> {
                        #body
                    }
                };
            }
            quote! {
                #[doc = #description]
                fn #name(#argument_decls) -> stardust_xr_fusion::node::NodeResult<#return_type> {
                    #body
                }
            }
        }
        MemberType::Method => {
            ...
            let body = if let Some(interface_node_id) = &interface_node_id {
                quote! {
                    ...
                    let (serialized_data, fds) = stardust_xr_wire::flex::serialize(&data)?;
                    let (message, message_fds) = _client.message_sender_handle.method(#interface_node_id, #aspect_id, #opcode, &serialized_data, fds).await?.map_err(|e| stardust_xr_fusion::node::NodeError::ReturnedError { e })?.into_components();
            ...
            if interface_node_id.is_some() {
                return quote! {
                    #[doc = #description]
                    pub async fn #name(#argument_decls) -> stardust_xr_fusion::node::NodeResult<#return_type> {
                        #body
                    }
                };
            }
...
fn generate_argument_deserialize(
    ...
    match argument_type {
        ...
        ArgumentType::Vec(v) => {
            ...
            quote!(#name.into_iter().map(|a| Ok(#mapping)).collect::<Result<Vec<_>, stardust_xr_fusion::node::NodeError>>()?)
        }
        ArgumentType::Map(v) => {
            let mapping = generate_argument_deserialize("a", v, false);
            quote!(#name.into_iter().map(|(k, a)| Ok((k, #mapping))).collect::<Result<stardust_xr_wire::values::Map<String, _>, stardust_xr_fusion::node::NodeError>>()?)
        }
        _ => quote!(#name),
    }
}
...
fn generate_argument_serialize(
    ...
    if optional {
        ...
        return quote!(#name.map(|o| Ok::<_, stardust_xr_fusion::node::NodeError>(#mapping)).transpose()?);
    }
    ...
    match argument_type {
        ...
        ArgumentType::Vec(v) => {
            ...
            quote!(#name.iter().map(|a| Ok(#mapping)).collect::<stardust_xr_fusion::node::NodeResult<Vec<_>>>()?)
        }
        ArgumentType::Map(v) => {
            let mapping = generate_argument_serialize("a", v, false);
            quote!(#name.iter().map(|(k, a)| Ok((k, #mapping))).collect::<stardust_xr_fusion::node::NodeResult<rustc_hash::FxHashMap<String, _>>>()?)
        }
        _ => quote!(#name),
    }
}
